...
export async function getCVsStatus(): Promise<CVRecord[]> {
  try {
    ...
    const cvRecords: CVRecord[] = candidates.map(candidate => ({
      id: candidate.id,
      fileName: candidate.cvFileName || candidate.name, 
...
export async function getExtractedData(id: string): Promise<CandidateExtractedData | null> {
  try {
...
export async function getTopRecommendations(candidateId: string): Promise<string[] | null> {
  try {
    ...
    if (!response.ok) {
      ...
      const errorData = await response.json();
      throw new Error(errorData.message || 'Error al obtener datos del candidato.');
    }
    ...
    console.error(`Fallo al obtener recomendaciones para ID ${candidateId}:`, error);
    throw error;
  }
}

export async function getCandidatesSummary(): Promise<CandidateExtractedData[]> {
...
export async function updateCandidate(
  id: string,
  updatedData: CandidateExtractedData 
