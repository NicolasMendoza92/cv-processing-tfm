...
export function OfferMatcherModal({ open, onOpenChange, candidate }: Props) {
  ...
  useEffect(() => {
    if (!open || !candidate) return; // 👈 espera a que esté abierto Y haya candidato

    setLoading(true);
    setSummary(null); // 👈 limpia estado anterior
    setOffers([]);
    ...
    matchOffersAction({
      ...
      console.log("matchOffersAction res:", res); // 👈 para debuggear
      if (res.success) {
  ...
  }, [open, candidate]); // 👈 depende de AMBOS
  return (
          ...
          <div className="grid grid-cols-3 gap-4 mb-4 text-center">
            ...
            <div className="border rounded p-3">
              <p className="text-2xl font-bold">
                {(summary.bestMatchScore * 100).toFixed(0)}%
              </p>
